...
const stickerMap = new Map(
  wemoveStickers.map((sticker) => [sticker.id, sticker]),
);
...
export default function GlobalMeetingChat() {
  ...
  const openRoomMenu = useCallback(
    (event, type, room) => {
      ...
      if (
        type === "GROUP" &&
        Number(room?.hostUserId) === Number(user?.memberId)
      ) {
  ...
  const leaveSelectedRoom = useCallback(async () => {
    ...
    try {
      if (roomMenu.type === "GROUP") {
        ...
        setRooms((current) =>
          current.filter(
            (room) => Number(room.meetingId) !== Number(meetingId),
          ),
        );
        setSelectedMeetingId((current) =>
