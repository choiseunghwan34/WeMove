  ...
  return (
    <div className={styles.finderLayout}>
      <aside className={styles.sidebar}>
        <div className={styles.profileMini}>
          ...
          <div>
            ...
            <span>로그인하고 내 주변 모임에 참여해보세요</span>
          </div>
        </div>
        ...
        <section>
          ...
          {["전체 모임", "오늘 모임", "이번 주 인기", "초보 환영"].map((item, index) => (
            <button
              key={item}
              className={cx("sidebarButton", index === 0 && "sidebarButtonActive")}
              type="button"
            >
              <span>{item}</span>
              <small>{[328, 24, 41, 86][index]}</small>
            </button>
          ))}
        </section>
        ...
        <section>
          ...
          <button
            type="button"
            className={cx("sidebarButton", sport === "전체" && "sidebarButtonActive")}
            onClick={() => setSport("전체")}
          >
            <span>전체</span>
            <small>429</small>
          ...
          {sports.map((item) => (
            <button
              key={item.id}
              type="button"
              className={cx("sidebarButton", sport === item.name && "sidebarButtonActive")}
      ...
      <main>
        ...
        <section className={styles.filterPanel}>
          <div className={styles.tabs}>
            {["전체", ...sports.map((item) => item.name)].map((item) => (
              <button
                key={item}
      ...
      <aside className={styles.rightbar}>
        <section>
          <h3>인기 지역</h3>
          {["운정동", "야당동", "금촌동", "문산읍"].map((item, index) => (
            <p key={item}><b>{index + 1}</b>{item}<span>{[38, 27, 19, 13][index]}개</span></p>
          ))}
        </section>
        ...
        <section>
          ...
          <div className={styles.calendar}>
            {["월", "화", "수", "목", "금", "토", "일", "12", "13", "14", "15", "16", "17", "18"].map((day, index) => (
              <span
                key={`${day}-${index}`}
                className={[2, 4, 5, 9, 11, 12].includes(index) ? styles.calendarOn : ""}
              >
                {day}
              </span>
            ))}
        ...
        <section>
          <h3>실시간 인기 종목</h3>
          {["러닝", "풋살", "헬스", "배드민턴"].map((item, index) => (
            <p key={item}><b>{index + 1}</b>{item}<span>+{[12, 8, 5, 4][index]}</span></p>
          ))}
        </section>
        ...
        <section>
          ...
          <p className={styles.guideText}>
            제목, 장소, 날짜, 모집 인원을 구체적으로 작성하면 신청률이 높아집니다.
          </p>
          <Link to="/meetings/new">가이드 보기</Link>
        </section>
      </aside>
    </div>
  );
