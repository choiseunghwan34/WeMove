...
export default function MeetingCreatePage() {
    ...
    const {isAuthenticated} = useAuth(); //로그인 인증 상태 가져오기
    const inputRefs = useRef({}); //ref객체 관리
    ...
    const handleSubmit = (e) => {
        ...
        for (const field of requiredFields) {
            // 값이 비어있을 때 어떤 필드인지 로그를 찍어봅니다.
            if (!form[field.key] || String(form[field.key]).trim() === "") {
                console.log("필수입력 필드 누락:", form[field.key], "필드명:", field.key);
                alert(`${field.label}을(를) 입력해주세요.`);
                inputRefs.current[field.refKey]?.focus();
                inputRefs.current[field.refKey]?.scrollIntoView({ behavior: 'smooth', block: 'center' });
                return;
            }
        }
        if(!form.sportId) {alert("운동 종목을 선택해주세요.");
            setIsSportModalOpen(true);
            return;}
        if(!form.regionId) {alert("지역을 선택해주세요.");
            setIsRegionModalOpen(true);
            return;}

        /*
        const optionalFields = ["image", "latitude", "longitude"]; // null허용
        const isFormValid = Object.entries(form).every(([key, value])=>{
            if(optionalFields.includes(key)) {return true}
            if (value === null || value === undefined || value === '') {
                console.log("필수 필드 누락(key): ", key);
                console.log("해당 필드의 값(value): ", value);
                return false;
            }
            return true;
        });
        ...
        * */
        ...
        // 사진이 있을 때만 formData에 추가 (사진 없으면 안 들어감)
