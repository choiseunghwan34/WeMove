...
WeMove는 사용자가 지역과 운동 종목을 기준으로 운동 모임을 찾고, 참가 신청하고, 모임장이 신청자를 승인/거절할 수 있는 지역 기반 운동 모임 매칭 플랫폼입니다.
...
- 마이페이지를 통해 내가 만든 모임, 신청한 모임, 참여한 모임을 확인
...
| 모임 | 모임 상세 | `/meetings/:id` | 모임 상세 정보 확인, 댓글/문의, 참가 신청 | 1차 |
...
| 폰트 | Pretendard |
| 반응형 | 모바일 햄버거 메뉴, 카드형 레이아웃, 가로 스크롤 최소화 |
...
- `docs/page-structure.md`: 페이지 구성
- `docs/erd.md`: ERD 및 DB 설계
- `docs/api-spec.md`: API 명세
- `docs/tech-stack.md`: 기술 스택
- `docs/setup-guide.md`: 실행 및 설정 가이드
